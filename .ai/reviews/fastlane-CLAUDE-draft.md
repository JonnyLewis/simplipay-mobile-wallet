...
**Total Lanes:** 18 (8 Android public + 5 iOS public + 5 internal)
...
4. [Internal Lanes](#internal-lanes)
...
**⚠️ No Validation:** Doesn't check if beta release exists before promoting.
...
Documented here to aid debugging of version generation or release note issues.

| Lane | Platform | Purpose |
|------|----------|---------|
| `generateVersion` | Android | Generates version string; reads from Gradle (`versionFile` task) or falls back to Git tag + commit count |
| `generateReleaseNote` | Android | Generates release notes from last commit |
| `generateFullReleaseNote` | Android | Generates extended release notes |
| `increment_version` | iOS | Gets version from Gradle, sets it in Xcode project, increments build number |
...
Located at `fastlane-config/project_config.rb`.
...
2. **`ProjectConfig::IOS`** (App-specific):
   - `app_identifier`: iOS app bundle ID
