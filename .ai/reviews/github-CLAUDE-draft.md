...
**⚠️ Known Issue:** No validation that beta release exists before promotion.
...
The following workflows exist in `.github/workflows/` and have been reviewed
at the header level. They are not yet fully documented. Do not modify without
reading the workflow file first.

| Workflow file | Name | Trigger | Purpose |
|--------------|------|---------|---------|
| `build-and-deploy-site.yml` | *(no explicit name)* | `workflow_call` | Builds Kotlin/JS web app and deploys to GitHub Pages. Intended to be called from other workflows. Requires `web_package_name` input. |
| `cache-cleanup.yaml` | Cleanup Cache | `pull_request`, `workflow_dispatch` | Gradle/CI cache cleanup on PRs and manual trigger. |
| `monthly-version-tag.yml` | *(no explicit name)* | Schedule (1st of month, 3:30 AM UTC), `workflow_dispatch` | Creates monthly version tags in format `YYYY.MM.0` using the reckon versioning strategy. |
| `sync-dirs.yaml` | Sync CMP Directories | Schedule + `workflow_dispatch` | Syncs directories from a configurable upstream repository. |
| `tag-weekly-release.yml` | *(no explicit name)* | Schedule (Sundays, 4:00 AM UTC), `workflow_dispatch` | Creates weekly version tags using the Gradle Reckon plugin and triggers the multi-platform build workflow. |
| `upload-demo-app-on-firebase.yaml` | Upload Demo App on Firebase | `workflow_dispatch`, `pull_request` | Uploads the demo APK to Firebase App Distribution. Accepts tester groups as input. |
...
**⚠️ Known Issue:** `set +e` swallows versionFile errors — build may proceed
with a missing or stale version.
...
**⚠️ Known Issue:** No validation that beta release exists before promoting.
...
Use `keystore-manager.sh` (repo root) to encode secrets:
...
**Fix:** Fastlane automatically sanitizes versions. Format: `YYYY.M.CommitCount`.

---

#### 5. Production promotion fails with "No beta release found"

**Cause:** Beta track empty (no pre-flight validation)
