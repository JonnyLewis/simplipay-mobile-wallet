...
│  ├─→ Read plans/YYYY-MM-DD-*.md       → Get plan details (per index)        │
...
```
╔══════════════════════════════════════════════════════════════════════════════╗
║  MIFOSPAY MOBILE WALLET - PLAN STATUS                                        ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
|:-:|------|:--------:|--------------|--------------|
| 1 | design-mockup | [████████░░] 80% (8/10) | Step 9: send-money mockups | 2026-01-05 |
| 2 | testing-auth | [████░░░░░░] 40% (4/10) | Step 5: LoginViewModel tests | 2026-01-05 |
| 3 | feature-payments | [██░░░░░░░░] 20% (2/10) | Step 3: Create PaymentsViewModel | 2026-01-04 |
...
| 5 | ✅ | Done | Generate payments mockups |
| 6 | ✅ | Done | Generate transfer-intrabank mockups |
| 7 | ✅ | Done | Generate transfer-interbank mockups |
| 8 | ✅ | Done | Generate notification mockups |
| 9 | 🔄 | **Current** | Generate send-money mockups |
| 10 | ⬜ | Pending | Generate autopay mockups |

---

## Current Step Details

### Step 9: Generate send-money mockups

**Target**: `design-spec-layer/features/send-money/mockups/`

**Tasks**:
- [ ] Run `/design send-money mockup`
...
### Step 10: Generate autopay mockups

**Target**: `design-spec-layer/features/autopay/mockups/`

Same process as Step 9 but for the autopay feature.
...
| 2026-01-05 | 9 | Started send-money mockups |
| 2026-01-04 | 5-7 | Completed transfer mockups |
...
| Action | Command |
|--------|---------|
| Execute current step | `/design send-money mockup` |
...
| `plans/YYYY-MM-DD-[type]-[desc].md` | Plan details | steps[], currentStep, progress |

> Plans live flat in `plans/` with date-prefixed filenames. There is no `active/`/`completed/` split — a plan's lifecycle is tracked by its `Status:` field and by which table it sits in within `PLANS_INDEX.md`.

---

## Plan File Structure

When `/gap-planning` creates a plan, it saves to `plans/YYYY-MM-DD-[type]-[desc].md`:
...
- [ ] **Step 9**: Generate send-money mockups ← CURRENT
  - Run `/design send-money mockup`
  - Files: `features/send-money/mockups/`

- [ ] **Step 10**: Generate autopay mockups
  - Run `/design autopay mockup`
  - Files: `features/autopay/mockups/`
...
| 2026-01-05 | 9 | 🔄 Started | send-money in progress |
```
...
2. For each plan listed under Active Plans, read `plans/[plan].md`
...
1. Read `plans/[plan].md`
2. Update its `Status:` field to "✅ Completed"
3. Move its row from Active Plans to Completed Plans in `plans/PLANS_INDEX.md` (the file stays in place — no `completed/` directory)
...
# → Creates plans/2026-06-23-design-mockup.md with 10 steps
...
# Edit plans/2026-06-23-design-mockup.md, mark step 1 as [x]

# 5. Check status again
/gap-status 2026-06-23-design-mockup
...
# 7. Mark complete
/gap-status complete 2026-06-23-design-mockup
# → Sets Status to Completed; moves its row to Completed Plans in PLANS_INDEX.md
```
...
| Selection | Action |
|-----------|--------|
| Execute current step | Run the step's command (e.g., `/design send-money mockup`) |
