...
See `CLAUDE.md` → **Cross-Platform Parity**. A feature is not complete until it exists and builds on **both iOS and Android**.

- Shared code (`commonMain`) generated by the phases below compiles into both apps automatically — that part is parity-free.
- The risk is **platform-shell work**: any `expect`/`actual` pair, Swift `AppDelegate` wiring, `Info.plist`/`AndroidManifest.xml`, `Podfile`/`build.gradle.kts`, or platform DI. If the feature needs *any* of these on one OS, it needs the mirror on the other in this same `/implement` run.
- Run **Phase 6: Platform Parity** (below) before reporting complete. Do not close a feature with only one platform done.
...
│                                                                              │
│  PHASE 6: PLATFORM PARITY (MANDATORY)   Both shells or it isn't done        │
│  ├─→ Did this feature touch any expect/actual, Swift, plist, manifest,      │
│  │   Podfile, Gradle, or platform DI?   → If no: parity is automatic (skip) │
│  ├─→ iOS shell artifact present+builds  → cmp-ios / iosMain / nativeMain    │
│  ├─→ Android shell artifact present+builds → cmp-android / androidMain      │
│  ├─→ Verify runtime on BOTH             → simulator + emulator              │
│  └─→ Any deferred side → log as open parity gap in CURRENT_WORK.md          │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
## PHASE 6: Platform Parity (MANDATORY)

The `commonMain` code from Phases 2–4 is already cross-platform. This phase closes the gap for anything that is **not** in `commonMain`.

### Step 6.1: Detect whether platform-shell work is needed

Parity work is required if this feature added or changed **any** of:

| Trigger | iOS location | Android location |
|---------|--------------|------------------|
| `expect`/`actual` declaration | `*/src/iosMain`, `*/src/nativeMain` | `*/src/androidMain` |
| Native permission / capability | `Info.plist`, `*.entitlements` | `AndroidManifest.xml` |
| Platform SDK / dependency | `Podfile` | module `build.gradle.kts` / convention plugin |
| App-shell wiring | `cmp-ios/iosApp/*.swift` (`AppDelegate`) | `cmp-android/.../MainActivity.kt`, `MifosPayApp.kt` |
| Platform-provided singleton in DI | iOS DI actual | Android DI actual |

If **none** apply, the feature is pure `commonMain` → parity is automatic. State that and skip to the report.

### Step 6.2: Mirror the artifact on the other platform

For each platform-shell change above, implement the equivalent on the opposite OS **now**, in this run. Use the existing counterpart as the reference pattern (e.g. an `androidMain` actual mirrors the `iosMain`/`nativeMain` one, a `MainActivity` wiring mirrors the `AppDelegate` one).

### Step 6.3: Build & verify both

```bash
# Android
./gradlew :cmp-android:assembleDemoDebug        # verify REAL exit code — do not pipe through tail
# iOS
xcodebuild -workspace cmp-ios/iosApp.xcworkspace -scheme cmp-ios \
  -configuration Debug -destination 'id=<sim-udid>' -derivedDataPath cmp-ios/build/dd build
```

Verify behaviour on **both** a simulator and an emulator where the feature has a runtime surface.

### Step 6.4: Log any deferred side

If one platform genuinely can't be finished in this run (e.g. blocked on an external credential), record it as an **open parity gap** in `claude-product-cycle/CURRENT_WORK.md` with the reason and the blocking item. A silent one-platform "done" is not allowed.
...
║     ├─ Platform-shell work needed?  [no → pure commonMain | yes]              ║
║     ├─ iOS:     [n/a | ✅ built | ⚠️ open gap: reason]                        ║
║     └─ Android: [n/a | ✅ built | ⚠️ open gap: reason]                        ║
║                                                                                ║
╠═══════════════════════════════════════════════════════════════════════════════╣
║  Next steps:                                                                   ║
