...
description: Retrofit the codebot kit onto an existing brownfield codebase at a given local path - survey the code, install the shared contract, workflows, SDLC, and per-directory context tree there. Claude Code wrapper for the shared import-codebase workflow.
---
...
- Hard gates first: target must be a clean git repo; never blind-overwrite existing `AGENTS.md`, `CLAUDE.md`, `.claude/`, or scripts - merge per the workflow's collision policy and show diffs before writing.
